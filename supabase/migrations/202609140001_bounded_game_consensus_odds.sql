-- Compute current consensus only for explicitly requested games.
...
as $$
  ...
  latest_bookmaker_snapshots as (
    ...
    from public.odds
    join requested_games using (game_id)
...
comment on function public.get_game_consensus_odds(integer[]) is
  'Returns current full-game consensus odds after restricting source scans to requested game IDs';
