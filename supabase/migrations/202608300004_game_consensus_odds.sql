-- Collapse the latest full-game lines into one bookmaker-neutral consensus per game.
create or replace view public.game_consensus_odds
...
latest_bookmaker_snapshots as (
  ...
  group by odds.game_id, odds.bookmaker_id
),
latest_market_rows as (
  select
    ...
    odds.odd
  from public.odds
  join latest_bookmaker_snapshots
    on latest_bookmaker_snapshots.game_id = odds.game_id
    and latest_bookmaker_snapshots.bookmaker_id = odds.bookmaker_id
    and latest_bookmaker_snapshots.provider_updated_at = odds.provider_updated_at
  cross join target_markets
  where odds.bet_id in (target_markets.spread_bet_id, target_markets.total_bet_id)
...
grant select on public.game_consensus_odds to anon, authenticated;

comment on view public.game_consensus_odds is
  'Median current full-game spread and total after selecting each bookmaker most balanced latest line';
